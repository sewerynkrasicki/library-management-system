    ...
    private void addLibrarianActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addLibrarianActionPerformed
        try {
            ...
            else if((!Pattern.matches("([a-zA-Z0-9_\\-\\.]+)@([a-zA-Z0-9_\\-\\.]+)\\.([a-zA-Z]{2,5})$",  ef) || ef.length() > 255))
            {
                ...
                bibld.createLibrarian(fn, sn, lf, pf, ef, a.getId(), r.getId());
                readerTable.setModel(dtm(bibld.readLibrarian()));
                setLibrarianTableWidth();
            }
        } catch (Exception ex) {
        ...
        try {
            if(readerTable.getSelectionModel().isSelectionEmpty()){
                ...
                else if((!Pattern.matches("([a-zA-Z0-9_\\-\\.]+)@([a-zA-Z0-9_\\-\\.]+)\\.([a-zA-Z]{2,5})$",  ef) || ef.length() > 255))
                {
                    ...
                    bibld.updateLibrarian(bibl, fn, sn, lf, pf, ef, a.getId(), r.getId());
    ...
    private DefaultTableModel dtm(List<Bibliotekarz> bibl){
        ...
        for(Bibliotekarz b:bibl){
            String[] row = {String.valueOf(b.getId()), b.getImie(), b.getNazwisko(), b.getLogin(), b.getHaslo(), b.getEmail(),
                    String.valueOf(b.getAdres().getMiasto()+ " " + b.getAdres().getUlica() + " " + b.getAdres().getNumerDomu()), String.valueOf(b.getRola().getUserType())};
            tab.addRow(row);
        }
        return tab;
    }
    
    public void setLibrarianTableWidth(){
